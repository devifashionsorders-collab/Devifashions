// server.js - final production-ready with Cloudinary + S3 + local disk fallback
...
app.post('/api/products', (req, res, next) => {
  // we route to the appropriate multer handler by checking env
  if (CLOUDINARY_CONFIGURED || s3Client) {
    // mem storage (we will upload buffers to cloud)
    return uploadMemory.array('images', 20)(req, res, (err) => {
      if (err) return res.status(500).json({ error: 'multer error', details: err.message });
      // continue to handler
    ...
    // local disk storage
    return uploadDisk.array('images', 20)(req, res, (err) => {
      if (err) return res.status(500).json({ error: 'multer error', details: err.message });
      try {
